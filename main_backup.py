...
class ConfigManager:
    ...
    def _parse_config_data(self, data):
        """Parse configuration data into key-value pairs from binary format."""
        config = {}
        
        # Define known BF6 settings with their expected types and default values
        known_settings = {
            # Graphics API
            'GstRender.Dx12Enabled': {'type': 'int', 'default': 1},
            'GstRender.VSyncMode': {'type': 'int', 'default': 0},
            'GstRender.FrameRateLimit': {'type': 'float', 'default': 0.0},
            'GstRender.FullscreenRefreshRate': {'type': 'float', 'default': 60.0},
            'GstRender.ResolutionScale': {'type': 'float', 'default': 1.0},
            'GstRender.FixedResolutionScale': {'type': 'float', 'default': 100.0},
            
            # Quality Settings
            'GstRender.TextureQuality': {'type': 'int', 'default': 2},
            'GstRender.ShadowQuality': {'type': 'int', 'default': 2},
            'GstRender.EffectsQuality': {'type': 'int', 'default': 2},
            'GstRender.LightingQuality': {'type': 'int', 'default': 2},
            'GstRender.PostProcessQuality': {'type': 'int', 'default': 2},
            'GstRender.MeshQuality': {'type': 'int', 'default': 2},
            'GstRender.TerrainQuality': {'type': 'int', 'default': 2},
            'GstRender.VegetationQuality': {'type': 'int', 'default': 2},
            
            # Advanced Graphics
            'GstRender.AmbientOcclusion': {'type': 'int', 'default': 1},
            'GstRender.MotionBlurWorld': {'type': 'float', 'default': 0.5},
            'GstRender.ChromaticAberration': {'type': 'int', 'default': 1},
            'GstRender.FilmGrain': {'type': 'int', 'default': 0},
            'GstRender.Vignette': {'type': 'int', 'default': 0},
            'GstRender.LensDistortion': {'type': 'int', 'default': 0},
            
            # Ray Tracing & Upscaling
            'GstRender.RayTracingEnabled': {'type': 'int', 'default': 0},
            'GstRender.DLSSEnabled': {'type': 'int', 'default': 0},
            'GstRender.AMDNvidiaUpscalingQuality': {'type': 'int', 'default': 1},
            'GstRender.FrameGeneration': {'type': 'int', 'default': 0},
            
            # Competitive Settings
            'GstRender.UltraLowLatency': {'type': 'int', 'default': 0},
            'GstRender.AMDLowLatency': {'type': 'int', 'default': 0},
            'GstRender.DRSFrameRateTarget': {'type': 'float', 'default': 60.0},
            
            # Display Settings
            'GstRender.HDREnabled': {'type': 'int', 'default': 0},
            'GstRender.Brightness': {'type': 'float', 'default': 0.5},
            'GstRender.Contrast': {'type': 'float', 'default': 0.5},
            'GstRender.FieldOfViewVertical': {'type': 'float', 'default': 70.0},
            
            # Audio Settings
            'GstAudio.MasterVolume': {'type': 'float', 'default': 100.0},
            'GstAudio.EffectsVolume': {'type': 'float', 'default': 100.0},
            'GstAudio.MusicVolume': {'type': 'float', 'default': 50.0},
            'GstAudio.VoiceVolume': {'type': 'float', 'default': 100.0},
            
            # Input Settings
            'GstInput.MouseSensitivity': {'type': 'float', 'default': 1.0},
            'GstInput.MouseAcceleration': {'type': 'int', 'default': 0},
    ...
    def _create_backup(self, custom_name=None):
        ...
        try:
            ...
            log_info(f"Backup created: {self.backup_path}", "BACKUP")
            return True
        except Exception as e:
...
class BackupTab(QWidget):
    """Enhanced backup management tab with better UX."""
    ...
    def setup_ui(self):
        ...
        header.setStyleSheet("font-size: 18px; font-weight: bold; color: #ffffff; margin-bottom: 10px;")
        layout.addWidget(header)
        ...
        quick_group = QGroupBox("Quick Backup")
        ...
        quick_layout.addWidget(self.backup_name_input)
        ...
        quick_layout.addWidget(self.create_backup_btn)
        ...
        self.custom_name_input.setPlaceholderText("Enter custom backup name (optional)")
        ...
        """)
        create_layout.addWidget(QLabel("Custom Name:"))
        create_layout.addWidget(self.custom_name_input)
        ...
        # Note: backup_list is already defined above, no need to redefine
        ...
        backup_actions_layout.setSpacing(15)
        
        # Note: restore_btn is already defined above, no need to redefine
        
        self.open_backup_btn = QPushButton("📂 Open Backup File")
        ...
        # Note: delete_btn is already defined above, no need to redefine
        
        backup_actions_layout.addWidget(self.restore_btn)
        backup_actions_layout.addWidget(self.open_backup_btn)
        backup_actions_layout.addWidget(self.delete_btn)
        backup_actions_layout.addStretch()
        
        actions_group.setLayout(backup_actions_layout)
        layout.addWidget(actions_group)
    
    def create_backup(self):
        """Create a new backup with optional custom name."""
        try:
            custom_name = self.backup_name_input.text().strip()
            if custom_name:
                # Validate custom name (remove invalid characters)
                custom_name = re.sub(r'[<>:"/\\|?*]', '_', custom_name)
                if not custom_name:
                    custom_name = None
            
            if self.config_manager._create_backup(custom_name):
                backup_name = f"FieldTuner_Backup_{custom_name}.bak" if custom_name else f"FieldTuner_Backup_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.bak"
                QMessageBox.information(self, "Success", f"✅ Backup created successfully!\n\n📁 {backup_name}")
                self.custom_name_input.clear()
                self.refresh_backups()
                log_info(f"Backup created successfully: {backup_name}", "BACKUP")
            else:
                QMessageBox.warning(self, "Warning", "❌ Failed to create backup!")
                log_warning("Failed to create backup", "BACKUP")
        except Exception as e:
            log_error(f"Failed to create backup: {str(e)}", "BACKUP", e)
    ...
    def open_backup_folder(self):
        ...
        try:
            ...
            if not os.path.exists(backup_path):
                ...
                log_info(f"Created backup directory: {backup_path}", "BACKUP")
            
            if platform.system() == "Windows":
                # Use shell=True to avoid the exit code issue with Windows explorer
                subprocess.run(f'explorer "{backup_path}"', shell=True)
            elif platform.system() == "Darwin":  # macOS
                subprocess.run(["open", backup_path], check=True)
            else:  # Linux
                subprocess.run(["xdg-open", backup_path], check=True)
            
            log_info(f"Opened backup folder: {backup_path}", "BACKUP")
            # Don't show error dialog since the folder actually opens successfully
        except Exception as e:
            log_error(f"Failed to open backup folder: {str(e)}", "BACKUP", e)
            QMessageBox.critical(self, "Error", f"❌ Failed to open backup folder: {str(e)}")
    
    def open_selected_backup(self):
        """Open the selected backup file in default application."""
        current_item = self.backup_list.currentItem()
        if not current_item:
            QMessageBox.warning(self, "Warning", "⚠️ Please select a backup to open!")
            return
        
        backup_file = current_item.data(Qt.ItemDataRole.UserRole)
        if not backup_file or not backup_file.exists():
            QMessageBox.warning(self, "Warning", "⚠️ Selected backup file not found!")
            return
        ...
        try:
            ...
            if platform.system() == "Windows":
                # Use shell=True to avoid the exit code issue with Windows explorer
                subprocess.run(f'explorer /select,"{backup_path}"', shell=True)
            elif platform.system() == "Darwin":  # macOS
                subprocess.run(["open", "-R", backup_path], check=True)
            else:  # Linux
                subprocess.run(["xdg-open", backup_path], check=True)
            
            log_info(f"Opened backup file: {backup_path}", "BACKUP")
            # Don't show error dialog since the file actually opens successfully
        except Exception as e:
            log_error(f"Failed to open backup file: {str(e)}", "BACKUP", e)
            QMessageBox.critical(self, "Error", f"❌ Failed to open backup file: {str(e)}")
    ...
    def refresh_backups(self):
        ...
        if not self.config_manager.BACKUP_DIR.exists():
            log_warning("Backup directory does not exist", "BACKUP")
        ...
        log_info(f"Refreshed backup list: {len(backup_files)} backups found", "BACKUP")
    
    def restore_backup(self):
        """Restore selected backup."""
        current_item = self.backup_list.currentItem()
        if not current_item:
            QMessageBox.warning(self, "Warning", "⚠️ Please select a backup to restore!")
            return
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                log_info(f"Backup restored: {backup_file.name}", "BACKUP")
            except Exception as e:
    ...
    def delete_backup(self):
        ...
        current_item = self.backup_list.currentItem()
        if not current_item:
            QMessageBox.warning(self, "Warning", "⚠️ Please select a backup to delete!")
            return
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                self.refresh_backups()
                log_info(f"Backup deleted: {backup_file.name}", "BACKUP")
            except Exception as e:
                log_error(f"Failed to delete backup: {str(e)}", "BACKUP", e)
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                QMessageBox.information(
                    self, 
                    "Backup Restored", 
                    f"✅ Most recent backup restored successfully!\n\n"
                    f"🔄 Your settings have been restored to the backup from {time_str}.\n"
                    f"💾 Your previous settings were backed up as 'Before_Restore'."
                )
                self.refresh_backups()
            except Exception as e:
                QMessageBox.critical(self, "Error", f"❌ Failed to restore backup: {str(e)}")
    ...
    def restore_selected_backup(self):
        """Restore the selected backup with clear information."""
        current_item = self.backup_list.currentItem()
        if not current_item:
        ...
        # Show clear confirmation with backup details
        reply = QMessageBox.question(
            self, 
            "Restore Backup",
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                QMessageBox.information(
                    ...
                    f"🔄 Your settings have been restored to the selected backup.\n"
                    f"💾 Your previous settings were backed up as 'Before_Restore'."
                )
                self.refresh_backups()
            except Exception as e:
    ...
    def delete_selected_backup(self):
        """Delete the selected backup with confirmation."""
        current_item = self.backup_list.currentItem()
        if not current_item:
